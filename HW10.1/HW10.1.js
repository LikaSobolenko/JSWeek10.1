...
for (heroe of heroes) {
  ...
  <figure class="img-container"><img src="${heroe.url}" alt="" /></figure>
  <div class="article-body">
  <h2>${heroe.name}</h2>
  <p>${heroe.universe}</p>
  <p>${heroe.alterego}</p>
  <p>${heroe.occupation}</p>
  <p>${heroe.friends}</p>
  <p>${heroe.superpowers}</p>
  <p>${heroe.info}</p>
  <div><span class="star fa fa-star fa-2x"></span>
        <span class="star fa fa-star fa-2x"></span>
        <span class="star fa fa-star fa-2x"></span>
        <span class="star fa fa-star fa-2x"></span>
        <span class="star fa fa-star fa-2x"></span>
  </div>
...
document.querySelector("section.articles").innerHTML = content;
...
for (let i = 0; i < stars.length; i++)
  (function (i) {
    stars[i].addEventListener("click", function () {
      ...
      let sibling = stars[i];
      //создаю массив всех последних звёздочек
      let siblings = [];
      while (sibling.previousSibling) {
        sibling = sibling.previousSibling;
        sibling.nodeType == 1 && siblings.push(sibling);
      }
      //расскрашиваю все звёздочки
